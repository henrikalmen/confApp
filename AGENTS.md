...
<!-- Project-specific prohibitions. Use the "Never X – [reason]" pattern – rules with rationale
     generalize better than bare prohibitions. -->
...
_**TODO**: Add further project-specific prohibitions as they emerge. Universal "never" rules already live in `docs/guidelines/CRITICAL-RULES-AND-GUARDRAILS.md`._
...
_**TODO**: Record the concrete commands/tooling once the app is scaffolded (see `docs/KEY_DEVELOPMENT_COMMANDS.md` → Visual Validation)._
...
_**TODO**: Add once the stack is pinned (React, build tool, and mobile-packaging docs)._
...
<!-- List project-specific tools and MCP servers here – especially CLI commands and servers that are
     ...
     ast-grep, tree, git, etc.) – agents already know them. -->
...
_**TODO**: No build tooling exists yet – the repository is empty. Fill in dev / test / lint / format / build commands once the app is scaffolded, including **how to run a single targeted test**._
