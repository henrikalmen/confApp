...
| ~~A refused renewal is split by code: session-lapse keeps the cache, grant-refusal purges~~ **Superseded 2026-08-26** by [ADR-005](../../adrs/ADR-005-bound-access-by-time-not-by-refusal-code.md) | The cost asymmetry it reasoned from was right; the premise was not. No refusal code distinguishes a deprovisioned Workspace account from a lapsed Workspace cookie – both return `login_required`, `invalid_grant` cannot reach an authorization redirect at all, and Google's own recommended mechanism (RISC) sends no events for Workspace users. The split was unimplementable, not merely mis-tuned | 2026-08-24 |
| No refusal ends a session; access is bounded by time | The ID token expires within about an hour and cannot be reissued to a suspended account, so the API closes to a departed employee regardless of client behaviour. The cached schedule is bounded by its readability window, now the earlier of `endDate + 7d` and `lastSync + 30d`; the stored session is bounded by `shared-device-session-lifetime`, which this promotes to the only mechanism that ends a stored session other than an explicit sign-out | 2026-08-26 |
| An unrecognised or transient refusal code keeps the cache **and no longer blocks renewal** (amended 2026-08-26, ADR-005) | The readability window already bounds exposure, so a misclassified deprovisioning still lapses within the margin; purging on a transient `server_error` would cost every attendee her schedule at once | 2026-08-24 |
