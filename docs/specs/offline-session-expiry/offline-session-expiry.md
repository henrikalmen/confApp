...
### Run: 2026-08-25 13:29 UTC – observations

#### ACCEPTED LIMITATION: the readability window is advisory against a tampered device clock

OC02 states that a cached schedule stops rendering once its conference span plus the margin has passed, "so a departed employee's offline access ends without depending on a reconnect". That holds against an honest device and not against a hostile one, and the code now says so at `web/src/offline/readability-window.ts` rather than claiming otherwise.

**Why:** the window is evaluated on S10's rehydrated effective clock, whose elapsed term reduces algebraically to `deviceClock() - anchor.deviceClockAtReceipt`. Only a clock wrong *at* the sync is cancelled by the anchor; a clock changed *after* the sync moves the effective day one-for-one, so setting the device back to the sync date makes a long-lapsed entry readable again indefinitely. `web/src/clock/effective-clock.ts` already documented this for the running-session highlight it was written for ("a device clock that jumps after sync skews the elapsed term ... which is accepted"); the same limit applies here and was not noticed when the window was built on top of it.

**Status:** open product decision, not an oversight. Closing it needs a monotonic high-water mark — the highest effective day ever observed, persisted with the cache entry and refused to go backwards. That is a new persisted field, which Structural Criterion 5 of this FIS forbids ("no new persisted field is added to `CachedSchedule`"), so it cannot be done inside this story's boundaries and needs the criterion amended or a follow-up story.
