...
- [ ] **S01 [OC01] [TI01,TI02,TI05,TI11] A role granted in one Conference has no effect in another**
...
- [ ] **S02 [OC01,OC02] [TI01,TI09] Presenting and facilitating are the same role with the same permissions**
...
- [ ] **S03 [OC02] [TI03,TI09,TI10] A Presenter/Facilitator edits only the Sessions assigned to them**
...
- [ ] **S04 [OC03] [TI02,TI04] Every Admin-only endpoint from S03, S04 and S05 refuses a mere Attendee**
...
- [ ] **S05 [OC03] [TI04,TI05,TI06,TI09] Any authenticated employee creates a Conference and is its first Admin**
...
- [ ] **S06 [OC04] [TI07,TI11] The last Admin cannot be removed, including by self-demotion**
...
- [ ] **S07 [OC01,OC04] [TI08,TI11] An employee who has never signed in cannot be assigned, and the stored key survives an email change**
...
- [ ] **S08 [OC04] [TI08] Roles cannot be changed on an archived Conference**
...
- [ ] Exactly one implementation of `requireConferenceRole` exists, its signature is unchanged from the provisional one S03 pinned, and no handler in S03's, S04's, S05's or this story's modules contains an inline creator or role comparison – every per-Conference authorization decision is a call to it.
- [ ] The role set has exactly three members – `Admin`, `PresenterFacilitator`, `Attendee` – in the TypeScript type, the API contract and the database check constraint; no `Presenter` or `Facilitator` member exists as a role anywhere in schema, code or wire format.
- [ ] Role Assignment and Session Assignment rows identify the user by `sub`; no column, foreign key, unique constraint, or lookup path keys the assignment on an email address.
- [ ] No authorization path reads a Google Workspace directory group, a `groups`/`hd`-derived group claim, or any external directory – the caller's role comes solely from confApp's own rows for the named Conference (ADR-002).
- [ ] Every Role Assignment row is scoped to exactly one Conference; no row, wildcard value, or configuration expresses an instance-level or cross-Conference role.
- [ ] Membership means "is in this Conference" for **every** role without exception, the creator included – no authorization path, member-listing query, or grant-target check treats an Admin as a member-by-implication rather than by a Membership row, and no role holder exists without one.
- [ ] The role check re-reads its rows per request and holds no role, membership, or permission cache in module, global, or static scope.
- [ ] Revoking a role leaves the user's Membership and every historical record intact – revocation removes an assignment only.
- [ ] Migrations are reversible and use plain PostgreSQL only, with no provider-specific extensions (ADR-003).
- [ ] Every refusal in this story emits through S01's JSON error envelope with a displayable message and a machine code distinct per refusal reason.
- [ ] The Admin's member-and-roles surface is legible with no horizontal body scroll at 375px, 768px and 1280px.
...
- [ ] **TI01** The three-role model is complete and Session Assignment links a Presenter/Facilitator to specific Sessions
...
- [ ] **TI02** `requireConferenceRole` resolves the caller's real effective role for the named Conference
...
- [ ] **TI03** `options.sessionId` narrows a Presenter/Facilitator to their assigned Sessions without widening an Admin
...
- [ ] **TI04** Every protected endpoint from S03, S04 and S05 declares its required role through the canonical check
...
- [ ] **TI05** An Admin grants and revokes Admin or Presenter/Facilitator for a member of their own Conference
...
- [ ] **TI06** Conference creation consults no instance-level permission
...
- [ ] **TI07** The last Admin of a Conference cannot be removed, including by self-demotion
...
- [ ] **TI08** Role changes are refused for an unresolvable target and on an archived Conference
...
- [ ] **TI09** An Admin assigns and unassigns a Presenter/Facilitator to specific Sessions of their Conference
...
- [ ] **TI10** Session write endpoints authorize an Admin or the Session's assigned Presenter/Facilitator
...
- [ ] **TI11** The Admin's member surface shows every member's roles and Session assignments and applies changes there
...
- [ ] **TI12** The member and roles surface is responsive across the three target widths
