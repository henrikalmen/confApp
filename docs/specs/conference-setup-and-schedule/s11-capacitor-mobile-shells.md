...
### Run: 2026-08-31 09:32 UTC – discovered-requirements

#### DISCOVERED REQUIREMENTS

_Propagated by the exec-plan wave triage from the facilitator-board-and-categorisation feature's S04 (Display Link issuance and revocation), 2026-08-31. This is a requirement discovered by another plan's story that binds this one. It changes nothing about this story's status or checkboxes._

- **S11 must set `webBaseUrl` in the Capacitor builds, alongside the absolute `apiBaseUrl` TI02 already owes.** S04 added a Display Link: a Facilitator issues an anonymous URL and a room machine opens it on a projector. The issue control has to state that URL, and inside the Capacitor shells `window.location.origin` is the WebView origin – `capacitor://localhost` on iOS, `https://localhost` on Android, both recorded in `web/capacitor.config.ts` – so a link built from it on the Facilitator's phone, which is the device the acceptance scenario names, is unopenable by any other machine while looking entirely plausible in the field. S04 therefore added `webBaseUrl` to the runtime-configuration contract (`web/public/config.js`, `web/docker-entrypoint.d/40-runtime-config.sh`, the Vite dev middleware) and `resolveWebBaseUrl` in `web/src/config.ts`. **It deliberately returns `null` rather than guessing an origin**, and the control then shows the raw token with a sentence saying it cannot state an address – a degraded but honest surface, chosen over emitting a broken URL. Until the shell build materializes `webBaseUrl` into `webDir` before `cap sync`, that degraded surface is what a Facilitator gets on Android and iOS. The container and the dev server need no value: a browser served over http(s) genuinely is reachable at its own origin.
