...
- [x] **S01 [OC01] [TI07,TI08,TI10,TI11] Browser renders a value read from PostgreSQL through the composed SPA and API containers**
...
- [x] **S02 [OC02] [TI05] Unknown API route is refused in the standard error envelope**
...
- [x] **S03 [OC02] [TI06] Invalid query parameter is rejected by the validation entry point before handler logic runs**
...
- [x] **S04 [OC02] [TI04] Database unavailability is reported as a refusal, not a crash or an internal-detail leak**
...
- [x] **S05 [OC03] [TI09] App shell is legible one-handed at 375px and rescales to tablet and desktop**
...
- [x] **S06 [OC04] [TI02,TI03,TI14] Migrations are reversible and leave a working schema after a full down/up cycle**
...
- [x] **S07 [OC01] [TI04] Consecutive requests to the same long-lived API process do not leak state between each other**
...
- [x] **S08 [OC05] [TI02,TI03,TI11] Data written before the database container is destroyed is read back unchanged after the container is recreated**
...
- [x] **S09 [OC05] [TI11,TI14] `docker compose down -v` deliberately discards state, and the command reference says so before a developer finds out**
...
- [x] **S10 [OC05] [TI11] Neither application container declares a volume, and neither needs one**
...
- [x] `npm run lint`, `npm run format:check`, `npm run typecheck`, `npm test` and `npm run build` all exit 0 on a clean checkout after `npm ci`.
- [x] No Azure Functions artifact exists anywhere in the tree – no `function.json`, no `host.json`, no `local.settings.json`, no `@azure/functions` dependency (ADR-004: no part of the application is written against the Functions programming model).
- [x] Neither image bakes in environment-specific configuration: database host/credentials, listen port, and the SPA's API base URL are all supplied at container run time, so the same image runs in development and against another target without a rebuild (S13 depends on this).
- [x] Migration SQL uses plain PostgreSQL only – no `CREATE EXTENSION`, no provider-specific types, functions, or DDL (ADR-003 portability).
- [x] `docker-compose.yml` declares a top-level **named** volume and the PostgreSQL service mounts it at the database data directory – not an anonymous volume, not a bind mount to a host path, and never left on the container's writable layer.
- [x] **No application service declares a volume of any kind** – neither the API nor the SPA service has a `volumes:` entry. This is a stated design property, not an omission: they keep no state between requests (`AGENTS.md#do-not--never`), which is exactly what makes the database the only component requiring durable storage.
- [x] No `.env` file or credential is committed; a tracked `.env.example` documents every required variable with non-secret placeholders.
- [x] `docs/STACK.md` contains no `_TBD_` entry for any component this story delivers, and each recorded version matches the committed lockfile / manifest / image tag.
- [x] `docs/KEY_DEVELOPMENT_COMMANDS.md` contains no `TODO`, including a working single-targeted-test command and the three visual-validation width commands.
...
- [x] **TI01** Repository builds from a clean clone as one npm workspace containing `web/` and `api/`, both TypeScript with strict mode on
...
- [x] **TI02** Local PostgreSQL starts from the composed stack with credentials supplied by an untracked `.env`, and its data lives on a named volume that outlives the container
...
- [x] **TI03** Schema is created and reverted by reversible migrations, with an initial migration creating and seeding the non-domain `app_meta` table
...
- [x] **TI04** Handlers reach PostgreSQL through one shared pooled data-access module that retains nothing request-derived between requests
...
- [x] **TI05** Every API refusal leaves the server in the standard error envelope, including unknown routes and unhandled exceptions
...
- [x] **TI06** Request input is validated at one shared entry point before handler logic runs, and rejections use the TI05 envelope
...
- [x] **TI07** `GET /api/health` answers from PostgreSQL, returning the seeded schema version and a server timestamp
...
- [x] **TI08** The API runs as a long-running HTTP server from its own image, configured entirely from the environment and shutting down without dropping in-flight requests
...
- [x] **TI09** The SPA presents a responsive app shell that renders the health result and reflows cleanly from 375px to 1280px
...
- [x] **TI10** The SPA's built static assets are served from their own container image whose API base URL is supplied at run time, not at build time
...
- [x] **TI11** One documented command brings the whole stack up from a clean state and the round trip works end to end
...
- [x] **TI12** Lint, format-check, type-check and test commands exist at the workspace root and pass on a clean tree
...
- [x] **TI13** `docs/STACK.md` records the resolved versions of everything this story ships, with no `_TBD_` left for delivered components
...
- [x] **TI14** `docs/KEY_DEVELOPMENT_COMMANDS.md` contains only commands that work as written, including running a single targeted test and the three visual-validation widths
...
- [x] No authentication code, no conference-domain table or entity, and no Capacitor project exists in the tree – those are S02, S03/S04 and S11, and landing them early would pre-empt decisions those stories own.
- [x] `GET /api/health` is the only route intended to remain anonymous after S02, and its payload carries nothing beyond liveness/readiness facts.
- [x] Nothing in the tree implements or documents a warm-up, pre-warm, or cold-start mitigation – ADR-004 removed the requirement rather than deferring it.
- [x] The named volume was proven by destroying and recreating the database container and reading the data back – not by inspecting `docker-compose.yml` – and nothing written in code, comments, or docs describes that volume as a backup.


## Implementation Observations

### Run: 2026-08-16 21:43 UTC – observations

#### NOTICED BUT NOT TOUCHING

- `AGENTS.md` → Standing Facts still describes a serverless Azure Functions backend and 'cold-start-tolerant handlers' – superseded by ADR-004. Flagged per this FIS's Constraints & Gotchas; owner-owned drift cleanup, deliberately not edited here.
- `docs/specs/conference-setup-and-schedule/prd.md` → Non-Functional Requirements still carries the cold-start carve-out and the pre-warm row. ADR-004 eliminated both rather than deferring them.
- `docs/specs/conference-setup-and-schedule/plan.json` → sharedDecisions 'API route, handler and error envelope conventions' still says 'Azure Functions HTTP route layout'. The conventions it names are what bind; the runtime it names is stale.

#### ASSUMPTIONS

- HTTP framework: Fastify 5.12.0. Its route-level JSON schemas are the single request-validation entry point (TI06), and setNotFoundHandler/setErrorHandler give one exit for every refusal (TI05).
- TypeScript pinned to 6.0.3, not the latest 7.0.2: typescript-eslint 8.67.0 declares `typescript >=4.8.4 <6.1.0`, so TS 7 would break `npm run lint`.
- Node pinned to 24 LTS in `.nvmrc` and in the API image; root `engines` is `>=24` so a newer locally installed runtime is not rejected.
- Migrations run through a thin programmatic wrapper (`db/migrate.mjs`) over node-pg-migrate, giving one cross-platform command shape and explicit `down all` semantics for S06.
- Host ports default to web 8082 / api 8081 / postgres 5434, chosen to avoid a native PostgreSQL on 5432 and other services already bound on this machine. All are `.env`-overridable.
- Integration tests skip loudly when no database is reachable, so `npm test` exits 0 on a clean checkout (Structural Criterion) while still running for real when the stack is up.

#### DESIGN DECISIONS TAKEN WITHIN FIS LATITUDE

- The SPA container resolves the API upstream per request (nginx `resolver` + a variable in `proxy_pass`) instead of at startup. With a literal hostname nginx refuses to start when the API is not yet resolvable, which took the whole SPA down with it and made the image unrunnable outside the compose network. Found by verification, not by review.
- `web` uses `depends_on: api: condition: service_started`, not `service_healthy`, so the SPA serves and shows the API's own refusal message rather than failing to load while the API is unready.
- A reachable-but-unmigrated database is reported as 503 DATABASE_UNAVAILABLE rather than 500. Handled in the health route only, so a mistyped relation in a later story still surfaces as INTERNAL_ERROR instead of being disguised as an outage.
- The named volume mounts at `/var/lib/postgresql`, the directory the postgres:18 image declares. PostgreSQL 18 moved PGDATA to `/var/lib/postgresql/18/docker`, so the pre-18 `/var/lib/postgresql/data` convention would have persisted an empty directory and lost the database on every container replacement.
