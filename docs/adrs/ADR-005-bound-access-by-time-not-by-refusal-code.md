...
**Status**: Accepted
**Date**: 2026-08-26 (proposed and accepted)
...
- One of the two OPEN reconciliation-ledger entries closes with the spec amendments this decision requires — the refusal-clearing/Structural-Criterion-4 entry. The TI05 entry (renewal trigger moved to the shell) is **not** touched by this decision and stays OPEN, to be amended in its own right.
...
- Amend **TI06**, **Acceptance Scenario S08**, **OC03**, **Structural Criterion 4**, **Structural Criterion 5**, **Structural Criterion 6** and the clarification's Decisions Log line, via the ADR-audited `design-change` form. Criterion 6 joins the list because the window predicate it governs gains a second horizon and a total fail-closed guard.
- Close the refusal-clearing entry in `docs/specs/offline-session-expiry/offline-session-expiry.reconciliation-ledger.md`; annotate the TI05 entry as out of this decision's scope.
- Record in `docs/specs/shared-device-session-lifetime/` that it is now the only bound on a stored session.

**Risks.** The 30-day horizon is a judgement, not a measurement — if conferences are routinely joined months ahead, it will bite legitimate attendees and should be raised. Eviction is irreversible, so a bug in the window predicate now destroys data rather than hiding it; the predicate's existing fail-closed path should be reviewed for the class of malformed values that currently answer "readable" before eviction is wired to it. **Discharged at acceptance**: every operand is shape-checked by `isCalendarDay` before any comparison, and that guard landed ahead of eviction.

**Acceptance record (2026-08-26).** Implemented in the same pass as acceptance: 871 tests green across 52 files, typecheck and lint clean, visual validation 3/3. All six decision points are in the code — `GRANT_REFUSED` deleted, `SESSION_LAPSED` gating the durable marker, `SYNC_MARGIN_DAYS = 30` as the second horizon, eviction at both observation sites. The superseding spec text is recorded under `## Implementation Observations` in the feature FIS.
